...
u_MaxTR = 0;
u_Max = 60;
u_T = 1 + rand(1,1) * u_MaxTR; % // taxythta pompou
u_R = 1 + rand(1,1) * u_MaxTR; % // taxythta dekth
...
u_scatterers = 1 + rand(num_scatterers,1) * u_Max;
...
% DTx = symbol_matrix(logical(pilot_grid));
% DTx = zeros(num_subcarriers, total_ofdm_symbols); 
% DRx = zeros(num_subcarriers, total_ofdm_symbols);
% 
% for m = 1:num_subcarriers
%     for n = 1:total_ofdm_symbols
%         DTx(m, n) = exp(1j * (2 * pi * rand(num_subcarriers, total_ofdm_symbols))); 
%     end
% end
% 
% yR = exp((-1j * 4 * pi * 120e3 * range_resolution * (0:num_subcarriers-1).') / c);
% yD = exp((1j * 4 * pi * Ts * freq * velocity_resolution * (0:total_ofdm_symbols-1)) / c);
% 
% for m = 1:num_subcarriers
%     for n = 1:total_ofdm_symbols
%         DRx(m, n) = DTx(m, n) * yR(m) * yD(n);
%     end
% end
% 
% DTx_ifft = ifft(DTx, num_subcarriers, 1); 
% 
% DTx_cp = [DTx_ifft(:, end-cplen+1:end); DTx_ifft]; 
% 
% serial_parallel = reshape(DTx_cp, num_subcarriers, []).';
% 
% DTx_ifft = FFT/sqrt(total_ofdm_symbols)*ifft(serial_parallel, FFT, 2);
% 
% add_cp = [signal(:, end-cplen+1:end) signal];
% add_cp = add_cp.';
% 
...
% qPeak = max(YV);
% 
% R = (c * pPeak) / (2 * 120e3 * num_subcarriers);
% v = (c * qPeak) / (2 * freq * Ts * total_ofdm_symbols);
% %%
D = zeros(size(DRx));
...
D_idft = ifftshift(ifft(D, [], 2));
D_dft = fftshift(fft(D_idft, [], 1));
...
for rx_ant = 1:num_elements_rx
    signal = abs(D_dft(:, :, rx_ant));
    for nn = 1 : num_of_sensing_signals
        [pks, locs] = findpeaks(signal(:,nn), 'MinPeakHeight', 0.001);
        if ~isempty(locs)
            keyboard
        end
        % plot(abs(signal(nn,:)))
