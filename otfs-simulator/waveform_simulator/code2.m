...
u_MaxTR = 0;
u_Max = 60;
u_T = 1 + rand(1,1) * u_MaxTR; 
u_R = 1 + rand(1,1) * u_MaxTR; 
...
u_scatterers = 1 + rand(num_scatterers,1) * 2*u_Max;
...
%         [pks, locs] = findpeaks(signal(:, nn), 'MinPeakHeight', thres);
%         % if ~isempty(locs)
%            % keyboard
%         % end
% 
% 
%         if abs(D_dft(rx_ant, nn)) > thres && abs(total_distance(rx_ant) - total_distance(nn)) <= 3
%             valid_peaks(rx_ant, nn) = D_dft(rx_ant, nn);
%         else
%             valid_peaks(rx_ant, nn) = 0; 
%         end
%         % plot(abs(signal(nn,:)))
%         % pause(0.5)
%     end
