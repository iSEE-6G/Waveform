...
%% LoS Path
steervec_los_tx = zero(num_ru, num_ue);
steervec_los_rx = zero(num_ru, num_ue);
...
for ll = 1 : num_ru
    ...
    steervec_los_tx(ll, :) = exp(-1j * (2 * pi * d_antenna_tx / lambda) * cos(gwnia_T));
    steervec_los_rx(ll, :) = exp(-1j * (2 * pi * d_antenna_rx / lambda) * cos(gwnia_R));
